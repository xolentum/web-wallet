...
public:
    ...
    static void exitHandler(int)
    {
        ...
        s_shouldExit = true;
        OMINFO << "Request to finish the OpenXolentum received";
        cv.notify_one();
    }
...
auto xolentum_log_level  =
        *(opts.get_option<size_t>("xolentum-log-level"));

auto verbose_level =
        *(opts.get_option<size_t>("verbose"));

if (xolentum_log_level < 1 || xolentum_log_level > 4)
{
    cerr << "xolentum-log-level,m option must be between 1 and 4!\n";
    return EXIT_SUCCESS;
}
...
mlog_set_log(std::to_string(xolentum_log_level).c_str());
...
OMINFO << "OpenXolentum is starting";
...
// with the xolentum deamon.
if (!current_bc_status->init_xolentum_blockchain())
{
...
// Publish the Open Xolentum API created so that front end can use it
